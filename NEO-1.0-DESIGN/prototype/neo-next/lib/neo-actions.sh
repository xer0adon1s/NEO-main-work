#!/usr/bin/env bash
# Validated action plans. No eval, no shell command strings, no implicit execution.
...
NEO_ACTION_POLICY="${NEO_ACTION_POLICY:-${NEO_NEXT_ROOT}/schemas/action-policy.json}"
...
neo_action_execute() {
    local file="$1" mode kind risk source timeout tool output rc=0 artifact=""
    local -a argv=()
    ...
    # AI commands require an explicit local policy allowlist. Built-ins are still shown and approved.
