#!/usr/bin/env bash
# Provider-neutral AI calls with secrets kept out of process arguments.
...
neo_provider_claude_cli() {
    ...
    prompt_file="$(neo_core_secure_tmp "${NEO_NEXT_STATE_ROOT}/tmp" provider-prompt)" || return 1
    {
        printf '%s\n\n--- OPERATOR DATA BOUNDARY ---\n' "$(cat -- "${system_file}")"
        cat -- "${user_file}"
    } > "${prompt_file}"

    # Explicitly remove API-key variables so subscription mode cannot silently bill an API key.
...
neo_provider_anthropic_api() {
    ...
    body="$(neo_core_secure_tmp "${NEO_NEXT_STATE_ROOT}/tmp" provider-body)" || return 1
    response="$(neo_core_secure_tmp "${NEO_NEXT_STATE_ROOT}/tmp" provider-response)" || { rm -f -- "${body}"; return 1; }
    config="$(neo_core_secure_tmp "${NEO_NEXT_STATE_ROOT}/tmp" provider-curl)" || { rm -f -- "${body}" "${response}"; return 1; }
...
neo_provider_extract_json() {
    ...
    # Accept a clean JSON object or a single fenced JSON object; reject surrounding prose.
