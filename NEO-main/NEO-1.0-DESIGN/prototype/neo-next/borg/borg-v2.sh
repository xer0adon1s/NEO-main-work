#!/usr/bin/env bash
# Borg v2: evidence -> initial dossier -> operator consent -> researched dossier.
...
source "${NEO_NEXT_ROOT}/lib/neo-provider.sh"
...
Builds a structured initial dossier from automated and operator recon, then asks whether
to assimilate selected vectors before foothold. Web research is used only when the selected
provider explicitly advertises that capability (NEO_PROVIDER_WEB_RESEARCH=1).
EOF
}
...
evidence_bundle="${work}/evidence.txt"
{
    ...
    if [[ -n "${NOTES_FILE}" ]]; then
        [[ -f "${NOTES_FILE}" && ! -L "${NOTES_FILE}" ]] || { neo_core_die 'notes file is invalid'; exit 1; }
...
if (( NONINTERACTIVE == 0 )); then
    printf 'Borg will send the prepared recon evidence to provider %s for an initial dossier.\n' "${NEO_AI_PROVIDER}"
...
research_user="${work}/research-user.txt"
{
    ...
    printf '\n# Initial dossier\n'; cat "${initial_json}"
    printf '\n# Evidence bundle\n'; cat "${evidence_bundle}"
    if [[ -f "${NEO_SOURCE_ROOT}/knowledge/resources/borg_research_index.yaml" ]]; then
        printf '\n# Curated research catalog\n'
        head -c 40000 "${NEO_SOURCE_ROOT}/knowledge/resources/borg_research_index.yaml"
    fi
} > "${research_user}"
...
printf '\nNo commands were executed. Review operator_actions, then convert accepted steps into validated action documents.\n'
