...
tmp="$(mktemp -d /tmp/neo-core-secrets.XXXXXX 2>/dev/null || mktemp -d)"
trap 'rm -rf -- "${tmp}"' EXIT
...
mode="$(stat -c '%a' "${NEO_SECRET_DIR}/ANTHROPIC_API_KEY" 2>/dev/null || stat -f '%OLp' "${NEO_SECRET_DIR}/ANTHROPIC_API_KEY")"
