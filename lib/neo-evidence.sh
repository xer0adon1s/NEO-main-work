#!/usr/bin/env bash
# Append-only JSONL evidence and hashed artifacts (C5).
...
neo_evidence_record() {
    ...
    local clean
    clean="$(neo_secret_redact_text "${summary}" ANTHROPIC_API_KEY OPENAI_API_KEY ANTHROPIC_WORKSPACE_ID)"
    jq -cn \
...
neo_evidence_save_artifact() {
    ...
    raw="$(cat)"
    clean="$(neo_secret_redact_text "${raw}" ANTHROPIC_API_KEY OPENAI_API_KEY ANTHROPIC_WORKSPACE_ID)"
    tmp="$(neo_core_secure_tmp "${NEO_EVIDENCE_DIR}/artifacts" ".artifact")" || return 1
    printf '%s' "${clean}" > "${tmp}"
    hash="$(sha256sum -- "${tmp}" 2>/dev/null | awk '{print $1}' || shasum -a 256 -- "${tmp}" | awk '{print $1}')"
